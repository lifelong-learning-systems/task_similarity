...
class MDPGraph:
    ...
    def from_grid(cls, grid, success_prob=0.9, reward=1, strat=ActionStrategy.NOOP_ACTION):
        P, R, out_s, out_a, available_actions, states_to_grid, grid_to_states = cls.grid_to_graph(grid, success_prob,
                                                                                                  reward=reward,
                                                                                                  strat=strat)
    ...
    def copy(self):
        return MDPGraph(self.P, self.R, self.out_s, self.out_a, self.available_actions, self.grid, self.strat,
                        self.states_to_grid, self.grid_to_states)
...
def compare_shapes(shape1, shape2, success_prob1=0.9, success_prob2=0.9, c_a=DEFAULT_CA, c_s=DEFAULT_CS,
                   strat=ActionStrategy.NOOP_ACTION):
...
def compare_shapes2_norm(shape1, shape2, success_prob1=0.9, success_prob2=0.9, c_a=DEFAULT_CA, c_s=DEFAULT_CS,
                         strat=ActionStrategy.NOOP_ACTION):
    return sim.normalize_score(compare_shapes2(shape1, shape2, success_prob1, success_prob2, c_a, c_s, strat=strat),
                               c_a, c_s)
