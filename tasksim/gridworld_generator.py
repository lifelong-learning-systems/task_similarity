...
class MDPGraph:
    ...
    def compare(self, other, c_a=DEFAULT_CA, c_s=DEFAULT_CS):
        G = self.append(other)
        from tasksim import structural_similarity
        S, A, num_iters, done = structural_similarity(G.P, G.R, G.out_s, c_a=c_a, c_s=c_s)
...
def compare_shapes(shape1, shape2, success_prob=0.9, c_a=DEFAULT_CA, c_s=DEFAULT_CS):
    return compare_graphs(MDPGraph.from_grid(create_grid(shape1), success_prob), \
                          MDPGraph.from_grid(create_grid(shape2), success_prob))
...
if __name__ == "__main__":
    ...
    score = final_score(compare_files(file1, file2))
    print(score, normalize_score(score))
