...
grid2 = grid1.copy()
grid2[0, 1] = 1
G3 = gen.MDPGraph.from_grid(grid1, success_prob=1, reward=10, strat=gen.ActionStrategy.NOOP_EFFECT_COMPRESS)
G4 = gen.MDPGraph.from_grid(grid2, success_prob=1, reward=10, strat=gen.ActionStrategy.NOOP_EFFECT_COMPRESS)
S_noop, num_iters_noop = G3.compare_song(G4)
import pdb; pdb.set_trace()




# print(f'Song: {sim.final_score_song(S_song)}')
# print(f'New: {sim.final_score(S_new)}')
tasksim.init_ray()
...
goal_locs = [(0, 6)]
STEP_REWARD = -0.01
STRAT = gen.ActionStrategy.NOOP_ACTION
